...
def chat_last_save():
    ...
    try:
        ...
        print('chat_last_save-에러--------------------------------------------------------', ex);    
    finally:
        print('chat_last_save-종료--------------------------------------------------------');    
        
    return NULL, NULL
        
# # 채팅방 커멘드 체크
def chat_chek_command(cls, clst):
    print('chat_chek_command-시작--------------------------------------------------------');        
    try:
        open_chatroom(kakao_opentalk_name)  # 채팅방 열기
        ttext = copy_chatroom(kakao_opentalk_name)  # 채팅내용 가져오기
        ...
        if df.iloc[-2, 0] == clst:
            ...
            if 1 <= int(found.count()):
                ...
                realtimeList = '과연 선제는 바보인가?'
                
                print(p_time_ymd_hms, realtimeList)
        ...
        print('chat_chek_command-에러--------------------------------------------------------', ex);        
        
    finally:
        print('chat_chek_command-종료--------------------------------------------------------');        
